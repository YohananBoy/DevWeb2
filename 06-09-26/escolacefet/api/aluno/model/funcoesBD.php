...
$listar = function () use ($pdo): array | null {
    $sql = "SELECT id, nome, nota1, nota2, media, grau
    FROM aluno";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
...
$alterar = function (array $aluno) use ($pdo): void {
    $sql = "UPDATE aluno SET nome = :NOME, nota1 = :NOTA1,
    nota2 = :NOTA2, media = :MEDIA, grau = :GRAU
    WHERE id = :ID";
    $stmt = $pdo->prepare($sql);
    $stmt->bindValue(':NOME', $aluno['nome'], PDO::PARAM_STR);
    $stmt->bindValue(':NOTA1', $aluno['nota1'], PDO::PARAM_STR);
...
$remover = function (int $id) use ($pdo): array | null {
