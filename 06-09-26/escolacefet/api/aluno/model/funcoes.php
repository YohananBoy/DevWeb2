...
function validar(array $aluno): void
{
    ...
    if (!isset($aluno["nome"], $aluno["nota1"], $aluno["nota2"]))
        responderJSON(["erro" => "Nem todos os arquivos vieram"], 400);
    if ($aluno["nome"] === "")
        responderJSON(["erro" => "O nome do aluno precisa ser preenchido"], 400);
    if (!is_numeric($aluno["nota1"]) && !is_numeric($aluno["nota2"]))
        responderJSON(["erro" => "As notas precisam de valores numéricos"], 400);
